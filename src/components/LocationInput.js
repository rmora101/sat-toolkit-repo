...
export const getStates = () => {
    ...
    const response = axios.get(`https://api.census.gov/data/2016/acs/acs1?get=NAME&for=state:*&key=${authToken}`)
        .then((response) => {
            const states = []
...
export const getIncomeRace = () => {
    const response = axios.get(`https://api.census.gov/data/2021/acs/acs1/profile?get=NAME,DP03_0062E,DP05_0078PE,DP05_0071PE,DP05_0037PE,DP05_0044PE&for=public%20use%20microdata%20area:00119&in=state:04&key=${authToken}`)
    .then((response) => {
        const stats = []
        for (let data of response.data) {
            ...
            const Asian = data[5]
            const IncomeAndRace = {'INCOME': Income, 'AAMERICAN': AAmerican, 'HISPANIC': Hispanic, 'CAUCASIAN': Caucasian, 'ASIAN': Asian }
            console.log(Income);
...
export const getPopulation = () => {
    const response = axios.get(`https://api.census.gov/data/2021/acs/acs1?get=NAME,B01001_001E&for=public%20use%20microdata%20area:00119&in=state:04&key=${authToken}`)
    .then((response) => {
        const population = []
        for (let data of response.data) {
